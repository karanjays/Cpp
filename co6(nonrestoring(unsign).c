...
//division of two unsigned binary numbers using NonRestoring Method.
#include<stdio.h>
...
int main(){
 ...
 scanf("%d",&len);
 char div[len],divi[len];
 printf("Please enter the value of divisor\n");
 scanf("%s",&div);
 printf("Please enter the value of Dividend\n");
 scanf("%s",&divi);
 for(int i=0,j=len-1;i<len;i++,j--) n+=(1ll<<j)*(div[i]-'0');
 for(int i=0,j=len-1;i<len;i++,j--) m+=(1ll<<j)*(divi[i]-'0');
 int ans[2];
